#!/usr/bin/env bash
# Claude Code → Codex 핸드오프. 새 터미널 창에서 codex를 첫 프롬프트와 함께 실행 (수동 호출/디버깅용)
# WARNING: 디버그용 사본. 정식 터미널 감지 체인은 plugins/relay-claude/commands/relay.md (3단계) 에 있음.
...
if   command -v ghostty             >/dev/null 2>&1; then setsid ghostty             -e bash -c "$CMD" </dev/null >/dev/null 2>&1 &
  ...
  echo "  cd \"$CWD\" && codex \"$INIT_PROMPT\"" >&2
  exit 1
fi
